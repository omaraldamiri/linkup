...
export default function TasksSummary() {
  const { workspaceTasks } = useTask();
  const { user } = useAuth();

  const now = new Date();

  const myTasks = workspaceTasks.filter((t) => t.assigneeEmail === user?.email);
  const overdueTasks = workspaceTasks.filter(
    (t) => t.dueTime && new Date(t.dueTime) < now && t.taskStatus !== "DONE",
  );
