...
const assigneeList = useMemo(
        () => Array.from(new Set(localTasks.map((t) => t.assignee?.name).filter(Boolean))),
        [localTasks]
    );
...
const handleStatusChange = async (taskId, newStatus) => {
    ...
    const handleDelete = async () => {
        ...
        setLocalTasks((prev) => prev.filter((t) => !selectedTasks.includes(t.id)));
        setSelectedTasks([]);
                    ...
                    <input
                      onChange={() =>
                        selectedTasks.length > 1
                          ? setSelectedTasks([])
                          : setSelectedTasks(localTasks.map((t) => t.id))
                      }
                      checked={selectedTasks.length === localTasks.length}
                      type="checkbox"
