...
import type { TaskDTO } from "../types/taskDtos";
...
const ProjectCalendar = () => {
  ...
  // Only tasks with a dueTime can appear on the calendar
  const tasksWithDue = tasks.filter((t): t is TaskDTO & { dueTime: string } =>
    Boolean(t.dueTime),
  );

  const getTasksForDate = (date: Date) =>
    tasksWithDue.filter((task) => isSameDay(new Date(task.dueTime), date));

  const upcomingTasks = tasksWithDue
    .filter(
      (task) =>
        !isBefore(new Date(task.dueTime), today) && task.taskStatus !== "DONE",
    )
    .sort(
          ...
          <div className="grid grid-cols-7 gap-2">
            {daysInMonth.map((day) => {
              ...
              const hasOverdue = dayTasks.some(
                (t) =>
                  t.taskStatus !== "DONE" &&
                  isBefore(new Date(t.dueTime), today),
              );
          ...
          <div className="not-dark:bg-white mt-6 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {getTasksForDate(selectedDate).map((task) => (
                <div
                  key={task.id}
                  className={`bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition p-4 rounded border-l-4 ${priorityBorders[task.taskPriority] ?? ""}`}
                >
                  <div className="flex justify-between mb-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs ${typeColors[task.taskType] ?? ""}`}
                    >
                      {task.taskType}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs text-zinc-600 dark:text-zinc-400">
                    <span className="capitalize">
                      {task.taskPriority.toLowerCase()} priority
                    </span>
              ...
              {upcomingTasks.map((task) => (
                ...
                >
                  <div className="flex justify-between items-start text-sm">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded ${typeColors[task.taskType] ?? ""}`}
                    >
                      {task.taskType}
                    </span>
                  </div>
                  <p className="text-xs text-zinc-600 dark:text-zinc-400">
                    {format(new Date(task.dueTime), "MMM d")}
                  </p>
                </div>
              ))}
            </div>
              ...
              {overdueTasks.slice(0, 5).map((task) => (
                ...
                >
                  ...
                  <p className="text-xs text-red-600 dark:text-red-300">
                    Due {format(new Date(task.dueTime), "MMM d")}
                  </p>
                </div>
              ))}
