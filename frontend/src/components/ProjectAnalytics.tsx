...
const ProjectAnalytics = () => {
  const { tasks } = useTask();
  const { currentProject, projectMembers } = useProject();
  ...
  const { stats, statusData, typeData, priorityData } = useMemo(() => {
    ...
    tasks.forEach((t) => {
      if (t.taskStatus === "DONE") stats.completed++;
      if (t.taskStatus === "IN_PROGRESS") stats.inProgress++;
      if (t.taskStatus === "TODO") stats.todo++;
      if (t.dueTime && new Date(t.dueTime) < now && t.taskStatus !== "DONE")
        stats.overdue++;

      if (statusMap[t.taskStatus] !== undefined) statusMap[t.taskStatus]++;
      if (typeMap[t.taskType] !== undefined) typeMap[t.taskType]++;
      if (priorityMap[t.taskPriority] !== undefined)
        priorityMap[t.taskPriority]++;
    });
  ...
  const metrics = [
    ...
    {
      label: "Team Size",
      // projectMembers is fetched by ProjectContext when currentProject changes
